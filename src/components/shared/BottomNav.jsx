...
const BottomNav = () => {
      ...
      const [guestCount, setGuestCount] = useState(false);
      const openModal = () => setIsModalOpen(true);
      const closeModal = () => setIsModalOpen(false);
      const navigate = useNavigate();

      const increment = () =>{if(guestCount > 6) return; setGuestCount((prev) => prev + 1);}
      const decrement = () =>{if(guestCount < 1) return; setGuestCount((prev) => prev - 1);} 
    ...
    return (
        <div className='fixed bottom-0 left-0 right-0 bg-[#262626] p-2 h-16 flex justify-around'> 
         <button onClick={() => navigate("/")} className='flex items-center justify-center text-[#f5f5f5] bg-[#343434] w-[200px] rounded-[20px]'>
          ...
          <button onClick={() => navigate("/orders")} className='flex items-center justify-center text-[#f5f5f5] bg-[#343434] w-[200px] rounded-[20px]'>
             <MdOutlineReorder className="inline mr-2" size={20} /><p>Orders</p>
          </button>
          <button onClick={() => navigate("/tables")} className='flex items-center justify-center text-[#f5f5f5] bg-[#343434] w-[200px] rounded-[20px]'>
          ...
          <Modal isOpen={isModalOpen} onClose={closeModal} title="Create Order">
            <div>
              ...
              <div className='flex items-center border border rounded-lg p-2'>
            ...
            <div>
              ...
              <div className='flex items-center border rounded-lg p-3 px-4 bg-[#1f1f1f]'>
                <input type="number" name="" placeholder='06' id='' className='bg-transparent flex-1 text-white focus:outline-none' />
              </div>
            </div>
            <div>
            ...
            <button onClick={() => navigate("/tables")} className='w-full bg-[#F6B100] text-[#f5f5f5] rounded-lg py-3 mt-8 hover:text-yellow-700'>
              Create Order
            </button>
